...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    // -------------------------------------------------------------
    // 3bis. MUTE / UNMUTE — relayé à tout le monde comme join/leave,
    //       pour que la pastille "micro coupé" se mette à jour même
    //       chez quelqu'un qui n'est pas encore dans ce salon vocal.
    // -------------------------------------------------------------
    if (msg.type === "voice-mute-state") {
      const { channelId, muted } = msg;
